...
<body class="haikei">
  ...
  <?php
    ...
    $db_review    = new GetReview();                                      //DBインスタンス生成
    $track_score  = $db_review->get_score( true, $_SESSION['comm_id'] );  //曲のIDと平均点取得
    $user_score   = $db_review->get_score( false, $_SESSION['comm_id'] ); //ユーザーのIDと平均点取得
  ?>
        ...
        <div id="track_ranking">
          <?php
            ...
            if(empty($track_score)){
                ...
                $track_id_rank = sortByKey( 'AVG(score)', SORT_DESC, $track_score );  //曲ランキングソート

                //ソート結果を順番に表示する
                foreach( $track_id_rank as $key => $value ){
                    if( $key >= 3 ) break; //3個目まで表示して終了
                    
                    $id_tmp = $value['spotify_id']; //URLとIDを結合
                    $spotify_url = "https://open.spotify.com/embed/track/"."$id_tmp";
                    $track_info = get_track_info($track_id_rank[$key]['spotify_id']); //ランキング配列内の曲情報取得
        ...
        <div id="user_ranking">
          <?php
            ...
            if(empty($user_score)){
                ...
                $user_id_rank = sortByKey('AVG(score)', SORT_DESC, $user_score ); //ユーザーランキングソート
